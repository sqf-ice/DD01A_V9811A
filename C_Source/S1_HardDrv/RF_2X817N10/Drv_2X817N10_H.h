...
**--------------file info--------------------------------------------
**name: MChip_RegDef_H.H
**Author: maji
**date: 
**description:  define for  measure chip function 
...
#ifndef _Drv_2X817N10_H
#define _Drv_2X817N10_H
...
#define VERSION        LSD4RF_2X717N10   
...
//             4     函数声明                                                              //
//******************************************************************
//extern void LSD_RF_StandbyMode(void);
//extern void LSD_RF_SleepMode(void);
//extern void LSD_RF_Config(void);
//extern void LSD_RF_RestRF(void);
//extern unsigned char LSD_RF_InitRF(void);
//extern void LSD_RF_RXmode(void);
//extern void LSD_RF_SendPacket(uint8 *txBuffer, uint8 size);
//extern void LSD_RF_RxPacket(uint8 *cRxBuf, uint8 *cLength);
//extern void LSD_RF_FreqSet(uint8 ch);
//extern void LSD_RF_WORInit(void);
//extern void LSD_RF_WORexecute(uint8 cLength);
//extern void LSD_RF_WOR_Exit(void);
//extern void LSD_RF_AwakePkt(void);
//extern void LSD_RF_TriggerRssi(void);
//extern unsigned char LSD_RF_GetRSSI(void);
//extern void LSD_RF_test(void);

//extern INT8U  Lnk_SX2176_Intilize();   // RF初始化
//extern INT8U Lnk_sx1276_rx_data(INT8U *rx_ptr);
...
** 函数宏定义                             **
*******************************************************************/
#define  LSD_RF_DELAY()            NOP()             //SPI延时

#define LSD_RF_SPIInit()        do{         \
                                        LSD4RF_SPI_SET_SDO_DIR_OUT() ;\
                                        LSD4RF_SPI_SET_SDO_HIGH(); \
                                            \
                                        LSD4RF_SPI_SET_CLK_DIR_OUT() ;  \
                                        LSD4RF_SPI_SET_CLK_LOW();  \
                                           \
                                        LSD4RF_SPI_SET_SDI_DIR_IN(); \
                                           \
                                        LSD4RF_SPI_SET_NSS_DIR_OUT() ; \
                                        LSD4RF_SPI_SET_NSS_HIGH();  \
                                             \
                                        LSD4RF_SPI_SET_DIO0_DIR_IN() ; \
                                        LSD4RF_SPI_SET_DIO1_DIR_IN() ; \
                                        LSD4RF_SPI_SET_DIO3_DIR_IN() ; \
                                  }while(0);

#define LSD_SPI_SIMO_OUT(x)    ( (x) ? (LSD4RF_SPI_SET_SDO_HIGH()) : (LSD4RF_SPI_SET_SDO_LOW()))	//  LSD_SPI_SIMO_PORT_OUT=x            //SIMO=X
#define LSD_SPI_SCK_OUT(x)     ( (x) ? (LSD4RF_SPI_SET_CLK_HIGH()) : (LSD4RF_SPI_SET_CLK_LOW()))            //sck=x
#define LSD_SPI_NSS_OUT(x)    ( (x) ? (LSD4RF_SPI_SET_NSS_HIGH()) : (LSD4RF_SPI_SET_NSS_LOW()))            //nss=x
#define LSD_SPI_SOMI_IN()     LSD4RF_SPI_READ_SDI()                

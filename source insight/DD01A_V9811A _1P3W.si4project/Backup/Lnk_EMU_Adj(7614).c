...
const  ST_EMU_ADJ_PARAM   code   default_adj_emu_param_tab = 
{
    ...
    0.000153309,  //float i1rms_xs;         //人为计算的通道1电流有效值系数  //
    0.000153309,  //float i2rms_xs;         //人为计算的通道2电流有效值系数  //8.279879e-005   1.14993e-007
    0,  //uint16     csck;  //校验和 //
};
...
void Calibration_Angular(float f_err_data)
{
	...
	//if(comm_data.di1_di0.UDWORD == 0x0000FEB1)    
	if(comm_data.di1_di0.u32 == 0x0000FEB1)    // DI03B  IA作为N回路  //0x0000FEB1 <-> 0x0000FEB4
	{
	...
	//else if(comm_data.di1_di0.UDWORD == 0x0000FEB4) 
...
void Calibration_small_signal(float f_err_data)
{
	...
	//if(comm_data.di1_di0.UDWORD == 0x0000FEB2) 
	if(comm_data.di1_di0.u32 == 0x0000FEB2)          // DI03B  IA作为N回路  //0x0000FEB2 <-> 0x0000FEB5
	{
	...
	//else if(comm_data.di1_di0.UDWORD == 0x0000FEB5) 
